...
export async function gifToMp4(url) {
  const id      = Date.now();
  ...
  await execAsync(
    `ffmpeg -i "${gifPath}" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -c:v libx264 -pix_fmt yuv420p "${mp4Path}"`
  );
...
const extractFromHtml = (html, baseUrl) => {
  ...
  for (const selector of selectors) {
    ...
    if (url && !url.includes("base64") && !url.startsWith("data:")) {
      ...
      if (url.match(/\.(jpg|jpeg|png|gif|webp|mp4|mkv|webm|mov|mp3|wav|ogg|m4a|flac)(\?|$)/i)) {
        return url;
      }
    }
  }
...
export const uploadToQuax = async (buffer) => {
  ...
  let mediaUrl =
    typeof data === "string"
      ? extractFromHtml(data, "https://qu.ax")
      : data.files?.[0]?.url;

  if (!mediaUrl) throw new Error("Upload failed");
  if (mediaUrl.includes("/x/")) return mediaUrl;

  const { data: pageHtml } = await axios.get(mediaUrl, {
